...
class TingoCollection extends MongooseCollection {
   ...
   afterInit(cb) {
      ...
      .on('end', () => {
         if (this.idx.length > 0 && Object.keys(this.idx[0]).length === 1) {
            this.rebuildIndex();
         }
         this.loading = false;
         this.loaded = true;
   ...
   getIndex(doc) {
      if (_.isEmpty(this.indexes)) return _.cloneDeep(doc);
   ...
   find(query, opts, _cb) {
      ...
      if (!_.isEmpty(keys)) {
         Promise.all(keys.map(_id => {
            return new Promise((res, rej) => {
               this.dataDb.get(_id, (err, doc) => {
                  if (err) return rej(err);
                  res(jsonfn.parse(doc));
               })
            })
         }))
         .then(docs => {
   ...
   findAndModify(query, sort, update, opts = {}, cb) {
      ...
      normalize(update);
      if (update && update.$setOnInsert) delete update.$setOnInsert;
      if (opts && opts.remove) return this.remove(query, opts, cb);
